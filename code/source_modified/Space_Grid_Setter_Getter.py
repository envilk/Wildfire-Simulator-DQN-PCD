...
# This import must have "agents.py" developed class in the same directory
from agents import Fire, UAV
...
class MultiGrid(_Grid):
    ...
    def get_copy(self):
        _new_grid_copy = []
        for _list in self._grid:
            _sublist_copy = []
            for _sublist in _list:
                # Inner appended list [...] corresponds to MultiGridContent (List[Agent], but since for Fire purposes,
                # this list only contains agents in the first position [0])
                _new_fire_agent = Fire(-1, None)
                _new_fire_agent.set_everything(*_sublist[0].get_everything())
                _sublist_copy.append([_new_fire_agent])
            _new_grid_copy.append(_sublist_copy)
        #print([int(fire[0].is_burning()) for sublist in _new_grid_copy for fire in sublist])
        #print('-----------------------------------------------------------')
        return _new_grid_copy

    def set_grid(self, _grid_copy):
        # Copied with its reference because previously is supposed to be deep copied
        self._grid = _grid_copy
    ...
    def grid_size(self):
        count = 0
        for sublist in self._grid:
            for subsublist in sublist:
                count += len(subsublist)
        return count

    def remove_UAVs(self) -> None:
        #Remove UAVs from grid.
        for _list in self._grid:
            for _sublist in _list:
                if len(_sublist) > 1:
                    for e in _sublist:
